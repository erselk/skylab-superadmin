    ...
    <AppShell>
      <div>
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold text-brand">Kullanıcılar</h1>
          <Link href="/users/new">
        ...
        {error ? (
          <div className="bg-light border border-dark-200 rounded-lg p-6">
            ...
            {error.includes('403') || error.includes('yetkiniz') ? (
              <div className="text-sm text-dark">
                ...
                {currentUser && (
                  <div className="mt-3 p-3 bg-light rounded border border-dark-200">
                    <p className="font-semibold mb-1 text-dark">Mevcut Rolleriniz:</p>
                    <p className="text-dark">{currentUser.roles.join(', ') || 'Rol atanmamış'}</p>
          ...
          <div className="bg-light border border-dark-200 rounded-lg p-6 text-center">
            <p className="text-dark opacity-60">Henüz kullanıcı bulunmamaktadır.</p>
          </div>
        ) : (
